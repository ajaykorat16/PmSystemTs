...
const ProjectList: React.FC<ProjectListProps> = ({ title }) => {
    ...
    const fetchProjects = async (currentPage: number, rowsPerPage: number, query: string, sortField: string, sortOrder: string) => {
        setIsLoading(true);
        let projectData;
    ...
    const hanldeSorting = async (e: DataTableStateEvent) => {
        ...
        setSortField(field);
        setSortOrder(order);
        fetchProjects(currentPage, rowsPerPage, globalFilterValue, field, order.toString());
    };

    const handleProjectDetail = async (project: any) => {
        setVisible(true);
        const developerNames = project.developers.map(
            (developer: any) => developer.id.fullName
        );
        setProject({
            name: project.name,
