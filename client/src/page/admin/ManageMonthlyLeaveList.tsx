...
interface LeaveManagementListProps {
    title: string;
}

const ManageMonthlyLeave: React.FC<LeaveManagementListProps> = ({ title }) => {
    ...
    const fetchLeaves = async (query?: number) => {
        setIsLoading(true);
        let leaveManagementData;
        if (!query) {
            leaveManagementData = await getLeavesMonthWise(currentPage, rowsPerPage);
        } else {
            leaveManagementData = await getLeavesMonthWise(currentPage, rowsPerPage, query);
        }
    ...
    useEffect(() => {
        fetchLeaves(parseInt(globalFilterValue, 10));
    ...
    const handleUpdate = async (id: string, fullName: string, monthly: number) => {
        setVisible(!visible);
        setLeaveId(id);
                            ...
                            <CForm onSubmit={addLeave}>
                                <CModalBody>
                                    ...
                                    <CFormInput
                                        ...
                                        id="leave"
                                        label="Manage Leave"
                                        value={manageLeave.leave.toString()}
                                        onChange={(e) => setManageLeave({ ...manageLeave, leave: parseFloat(e.target.value) })}
                                    />
                                </CModalBody>
                                <CModalFooter>
...
export default ManageMonthlyLeave;
